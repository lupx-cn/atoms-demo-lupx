...
- **三栏经典布局**：左会话列表 / 中 AI 对话 / 右统一多功能面板，三栏宽度可拖拽调节
- **预览 ⇄ 代码 Tab**：右栏模拟浏览器窗口预览（可拖拽/右手柄缩放），同一区域切换完整 HTML 源代码编辑器（可编辑/保存/下载）
- **原型导出**：一键下载当前版本 HTML 单文件（文件名=会话项目名-V{n}），双击即用任意浏览器独立打开
...
├── static/
│   ├── index.html       # 三栏页面骨架（左会话列表 / 中对话 / 右多功能面板）
│   ├── css/style.css    # 自定义样式
│   └── js/              # app/state/storage/api/preview/download/components(fileTree)
├── requirement_layout_v2.md  # 需求追加：三栏布局 + 多功能面板（v2）
...
### M4 增强 v2 · 三栏布局 + 右侧多功能面板（✅ 已实现）

> 依据 `requirement_layout_v2.md` 实现（需求已与用户逐项确认 Q1~Q5），完整需求与验收标准见该文档。

- [x] 三栏经典布局：左栏会话列表 / 中栏 AI 对话聊天 / 右栏统一多功能面板，三栏宽度可拖拽调节（宽度偏好记忆）
- [x] 右栏「网页预览 / 源代码」Tab 切换（替代原顶栏「查看代码」与底部弹出代码面板）
- [x] 预览模式：模拟浏览器窗口（地址栏/刷新按钮），画布内可拖拽位置 + 右手柄缩放，位置/缩放偏好记忆
- [x] 版本隔离：V1/V2 切换只更新右栏对应版本 HTML，AI 说明文字只保留在中栏聊天记录，预览区零混入
- [x] 代码模式：完整 HTML 源码编辑器（编辑 / 保存并刷新预览 / 下载）+ 文件资源管理器（index.html）
- [x] 导出下载：预览与代码模式内均可【下载 HTML】，文件名 = 会话项目名-V{n}.html，双击独立运行
- [x] 消息卡片化：assistant 生成结果改为版本卡片（版本号 + 时间 + 需求摘要 + 在预览中查看/复制/下载），聊天区不再铺大段源码
